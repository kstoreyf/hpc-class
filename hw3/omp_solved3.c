...
void print_results(float array[N], int tid, int section) 
{
  ...
  // BUG/FIX: We shouldn't have a barrier here! Only one thread at a time
  // is executing this function, so if we wait til they all reach the barrier,
  // we will end up hanging. Just remove this barrier to fix the bug.
  //#pragma omp barrier
